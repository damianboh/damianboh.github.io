...
////////////////////// start of functions for drawing graphs /////////////////////////
	...
	//###############DISPLACEMENT GRAPH
		...
		function drawDisplacementGraph() {
                ...
                current_cycle = Math.floor((getXPixel(t_previous) - xPadding) / (displacement_graph.width - xPadding ));
                ...
                if (getXPixel(t_current) <= displacement_graph.width){
	                ...
	                displacementContext.lineTo(getXPixel(t_current), getYPixel_displacement(h_current));
 					...
 					if(Math.abs((getXPixel(t_previous) - xPadding) % (displacement_graph.width - xPadding ) + xPadding)-((getXPixel(t_current) - xPadding) % (displacement_graph.width - xPadding ) + xPadding) < 100 ){
               			displacementContext.moveTo((getXPixel(t_previous) - xPadding) % (displacement_graph.width - xPadding ) + xPadding, getYPixel_displacement(h_previous));
	              
	                	displacementContext.lineTo((getXPixel(t_current) - xPadding) % (displacement_graph.width - xPadding ) + xPadding , getYPixel_displacement(h_current));
	                }
               	}
	...
	//###############ENERGY GRAPH
		...
		function labelAxisEnergy() {
                ...
                for(var i = 1; i < 4; i ++) {
                    ...
                    energyAxisContext.fillText((multiple_t_max*t_max/3*i).toPrecision(3), getXPixel(multiple_t_max*t_max/3*i), getYPixel_energy(0) + 20);
					// mark out the axis
